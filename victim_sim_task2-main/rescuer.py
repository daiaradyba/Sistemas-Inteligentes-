...
from classifier_regressor import *
...
class Rescuer(AbstAgent):
    def __init__(self, env, config_file, nb_of_explorers=1,clusters=[]):
        ...
        self.plan_rtime = 100  # the remaing time during the planning phase
    ...
    def save_cluster_csv(self, cluster, cluster_id):
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            for vic_id, values in cluster.items():
                ...
                #print(f"Cluster {cluster_id}: id{vic_id} ({x}, {y}) - {vs}")
                writer.writerow([vic_id, x, y, vs[5], vs[6]])
                #print(f"Cluster {cluster_id}: id{vic_id} ({x}, {y}) - {vs[3]} - {vs[4]}")
    ...
    def save_sequence_csv(self, sequence, sequence_id):
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            for id, values in sequence.items():
                x, y = values[0]      # x,y coordinates
                vs = values[1]        # list of vital signals
                writer.writerow([id, x, y, vs[5], vs[6]])
    
    def cluster_victims(self, balanced=True):
        """ 
        This method clusters victims based on distance and severity.
                
        @param balanced: boolean indicating whether clusters should have the same size
        @returns: a list of 4 clusters where each cluster is a dictionary in the format [vic_id]: ((x,y), [<vs>])
        ...
        vic = self.victims
        vic_list = list(vic.items())
        
        features = []
        for key, values in vic_list:
            x, y = values[0]
            severity = values[1][6]
            features.append([x, y])
        ...
        # KMeans clustering com inicialização k-means++
        kmeans = KMeans(n_clusters=4, init='k-means++', random_state=0).fit(features)
        ...
        if balanced:
            # Contar o número de vítimas em cada cluster
            cluster_counts = np.bincount(labels, minlength=4)
            target_size = len(vic) // 4
            excess_victims = []

            # Primeira alocação - adicionar vítimas respeitando o target_size
            for idx, vic_id in enumerate(vic.keys()):
                cluster_idx = labels[idx]
                if len(clusters[cluster_idx]) < target_size:
                    clusters[cluster_idx][vic_id] = vic[vic_id]
                else:
                    excess_victims.append((vic_id, vic[vic_id], features[idx]))

            # Redistribuição das vítimas excedentes com base na proximidade ao centroide e necessidade de balanceamento
            new_labels = np.copy(labels)
            for vic_id, vic_data, vic_coords in excess_victims:
                # Calcular distâncias para todos os centróides
                distances = [np.linalg.norm(vic_coords - centroid) for centroid in centroids]
                sorted_indices = np.argsort(distances)
                
                for cluster_idx in sorted_indices:
                    if len(clusters[cluster_idx]) < target_size + (len(vic) % 4 > 0):  # Permitir clusters maiores pelo resto
                        clusters[cluster_idx][vic_id] = vic_data
                        new_labels[list(vic.keys()).index(vic_id)] = cluster_idx  # Atualizar o rótulo
                        break
        else:
            # Distribuição simples baseada nos rótulos do K-Means
            new_labels = labels
            for idx, vic_id in enumerate(vic.keys()):
                cluster_idx = labels[idx]
                clusters[cluster_idx][vic_id] = vic[vic_id]

        # Plotar os clusters balanceados
        plt.figure(figsize=(10, 7))
        colors = ['r', 'g', 'b', 'y']
        for cluster_idx in range(4):
            cluster_points = features[new_labels == cluster_idx]
            plt.scatter(cluster_points[:, 0], cluster_points[:, 1], c=colors[cluster_idx], label=f'Cluster {cluster_idx + 1}')
        
        plt.title('Clusters de Vítimas')
        plt.xlabel('Coordenada X')
        plt.ylabel('Coordenada Y')
        plt.legend()
        plt.savefig('clusters_de_vitimas.png')
        plt.close()
    ...
    def predict_severity_and_class(self):
        ...
        regressor = joblib.load('models/mlp_model.pkl')
        ...
        for vic_id, values in self.victims.items():
            ...
            vs = values[1]
            #print(vs)
            vs_scaled = scaler.transform([vs[3:6]])
            predict = float(regressor.predict(vs_scaled)[0])
            values[1].pop(-1)
            values[1].extend([predict, 0])  # predict the severity value
            #print(f"Predicted severity value: {predict}")
            #vs.append(predict)  # predict the severity value
            #print(f"{self.NAME},{vs[0]} severity value: {vs[6]}")
            #vs.append(0)

    # Calcular a distância entre todas as vítimas entre si e com o agente, usar um Algoritmo Genético para encontrar a melhor sequência de resgate
    def sequencing(self):
        ...
        #print (f"{self.NAME} Sequences: {new_sequences}")
        self.sequences = new_sequences

    def ag_sequencing(self):

        new_sequence = []

        print(f"Initializing Sequencing for {self.NAME}")
        print(f"{self.NAME} AG Sequencing: {len(self.sequences)} sequences")
        #start_time = time.time()
        for seq in self.sequences:
            ga = GA(self.map, seq, self.plan_rtime, "manhattan", priority = True)
            ga.run(100, 200, 0.5, 0)
            best_route = ga.best

            aux_sequence = {}
            for vic in best_route:
                if vic[3] in list(seq.keys()):
                    aux_sequence[vic[3]] = seq[vic[3]]

            new_sequence.append(aux_sequence)
            

        #end_time = time.time()
        print(f"{self.NAME} sequence of visit:\n{best_route}")
        #print(f"{self.NAME} GA time: {end_time - start_time}\n")
    ...
    def planner(self):
        ...
        goal = (0,0)
        plan, time = bfs.search(start, goal)
        self.plan = self.plan + plan
    ...
    def sync_explorers(self, explorer_map, victims):
        ...
        print(f"{self.NAME} Victims received: {victims}")
        ...
        if self.received_maps == self.nb_of_explorers:
            ...
            #@TODO predict the severity and the class of victims' using a regressor
            self.predict_severity_and_class()

            #@TODO cluster the victims possibly using the severity and other criteria
            # Here, there 4 clusters
            clusters_of_vic = self.cluster_victims(True)
            ...
            for i, rescuer in enumerate(rescuers):
                rescuer.ag_sequencing()         # the sequencing will reorder the cluster
                ...
                #print(f"{rescuer.NAME} sequence of visit:\n{rescuer.sequences}\n")
                rescuer.planner()            # make the plan for the trajectory
                #print(f"{rescuer.NAME} plan: {rescuer.plan}")
